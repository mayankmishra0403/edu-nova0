...
function App() {
  ...
  const needsProfile = (() => {
    ...
    const avatarOk = !!(user.prefs && user.prefs.avatarFileId);
    const phoneOk = !!(user.prefs && (user.prefs.phone||'').trim());
    const rollOk = !!(user.prefs && (user.prefs.rollNo||'').trim());
    return !(nameOk && avatarOk && phoneOk && rollOk);
  })();
  ...
  // Add ripple effect only to elements explicitly marked to reduce overhead
  useEffect(() => {
  ...
  const handleTabChange = (newTab) => {
    if (isAuthed && newTab !== 'profile' && needsProfile) {
      newTab = 'profile';
    }
  ...
  return (
    <div>
      ...
      <button data-ripple="true" role="tab" className={`nav__tab ${tab === "home" ? "is-active" : ""}`} onClick={() => handleTabChange("home")}>
            <span className="nav__icon">🏠</span>
            <span className="nav__label">Home</span>
          </button>
      <button data-ripple="true" role="tab" className={`nav__tab ${tab === "ai" ? "is-active" : ""}`} onClick={() => handleTabChange(isAuthed ? "ai" : "profile")}>
            <span className="nav__icon">🤖</span>
            <span className="nav__label">EduAI</span>
          </button>
      <button data-ripple="true" role="tab" className={`nav__tab ${tab === "academics" ? "is-active" : ""}`} onClick={() => handleTabChange(isAuthed ? "academics" : "profile")}>
            <span className="nav__icon">📚</span>
            <span className="nav__label">Academics</span>
          </button>
      <button data-ripple="true" role="tab" className={`nav__tab ${tab === "placements" ? "is-active" : ""}`} onClick={() => handleTabChange(isAuthed ? "placements" : "profile")}>
            <span className="nav__icon">💼</span>
            <span className="nav__label">Placements</span>
          </button>
      <button data-ripple="true" role="tab" className={`nav__tab ${tab === "competitions" ? "is-active" : ""}`} onClick={() => handleTabChange(isAuthed ? "competitions" : "profile")}>
            <span className="nav__icon">🏆</span>
            <span className="nav__label">Competitions</span>
          </button>
          {isAdminUser && (
            <button data-ripple="true" role="tab" className={`nav__tab ${tab === "admin" ? "is-active" : ""}`} onClick={() => handleTabChange("admin") }>
              <span className="nav__icon">�️</span>
              <span className="nav__label">Admin</span>
            </button>
          )}
      <button data-ripple="true" role="tab" className={`nav__tab ${tab === "profile" ? "is-active" : ""}`} onClick={() => handleTabChange("profile") }>
            <span className="nav__icon">👤</span>
